...
- 全站音乐只使用 `GlobalControls.astro` 中一个持久化原生 `<audio>`；左下唱片球展开紧凑歌单，首页音乐卡只是同步控制视图。展开面板是承载密集控件的浮层工具，使用比首页内容卡更实的独立半透明底色，同时保留模糊、高光和无 `backdrop-filter` 回退，避免背景图穿透文字。列表循环、单曲循环和随机播放共用同一模式状态，并在刷新后保留用户选择；随机模式不得立即重复当前曲，单曲模式只在自然结束时重复，手动上一首/下一首仍应换曲。页面切换不得重建或暂停音频，不自动播放；网易云外链失败时明确降级，Reduced Motion 下封面不旋转。
...
- 归档页通过原生 Tabs 在博客笔记与个人轨迹之间切换，两套内容都使用语义化 `<ol>`。活动面板以紧凑的行内标题和计数开场，不使用全宽分隔线或仪表盘式大标题；主轴直接从第一条记录开始，不额外插入月份、当前阶段坐标牌、圆形徽章或横跨画面的装饰线，日期和阶段信息由记录卡的 meta 行承担。桌面航线固定为 `minmax(0, 1fr) 92px minmax(0, 1fr)` 三列，卡片等宽交替停靠，节点与卡片 meta 行对齐；`760px` 及以下统一降为 `68px` 左侧航道，主轴和节点位于轨道中央 `34px`。个人轨迹当前确认时间为软件工程学习 `2024.09 — 至今`、Heart Island 持续开发 `2026.04 — 至今`、AI 产品开发实习 `2026.06 — 至今`，并链接到真实项目与 `/campus/`。About 不再复制时间线，只展示真实头像、身份、介绍、事实和入口。
- `ArchiveVoyage.astro` 只在两套归档面板之后渲染一次，昼夜主题分别使用 `src/assets/images/archive/archive-voyage-day.png` 与 `archive-voyage-night.png`。它是无遮罩卡片外壳的视觉句号，必须向时间线方向重叠，并同时保留纵向和边缘渐隐、完整纸舟构图及四张主题候选图的懒加载；不得把参考图或文档截图重新接入页面，也不得用硬裁切或矩形边缘取代过渡。
...
首页卡片材质参考 XingHuiSama 的通用背景分层、半透明面板、柔和边缘和模糊景深原则，但由本站使用原生 CSS 和语义 token 独立实现。当前桌面主面板日间使用约 `14px / 112%`、夜间使用约 `18px / 116%` 的模糊与饱和度，并配合低饱和双层阴影和独立透明度；日间遮罩与面板更透，让背景保持可感知。移动端将大面积 `backdrop-filter` 收敛为约 `10px / 108%`，同时使用更实的半透明底色、边框和内高光保证可读性与性能回退。不得复制其 Tailwind 类、组件代码、精确色值、图片或功能；其上游仓库采用 CC BY-NC 4.0。
