...
- 归档页通过原生 Tabs 在博客笔记与个人轨迹之间切换，两套内容都使用语义化 `<ol>`。活动面板以紧凑的行内标题和计数开场，不使用全宽分隔线或仪表盘式大标题；章节标签固定为贴附主轴的路线坐标牌，不恢复圆形徽章和横跨画面的装饰线。桌面航线固定为 `minmax(0, 1fr) 92px minmax(0, 1fr)` 三列，卡片等宽交替停靠，节点与卡片 meta 行对齐；`760px` 及以下统一降为 `68px` 左侧航道，主轴和节点位于轨道中央 `34px`。个人轨迹当前确认时间为软件工程学习 `2024.09 — 至今`、Heart Island 持续开发 `2026.04 — 至今`、AI 产品开发实习 `2026.06 — 至今`，并链接到真实项目与 `/campus/`。About 不再复制时间线，只展示真实头像、身份、介绍、事实和入口。
