...
- `scripts/verify.mjs`：顺序执行 lint、Astro check、build 和结构契约
...
`studio` 在个人电脑启动本地写作后台和 Astro 草稿预览，不是生产服务。`verify` 依次运行 lint、Astro check、build 和结构契约；任一步失败即停止。Windows Git Bash 若无法直接调用 pnpm，可运行 `node scripts/blog-studio.mjs` 或 `node scripts/verify.mjs`。
...
- 所有修改至少运行 `pnpm run verify`，它必须完整通过 lint、Astro check 和 build。
